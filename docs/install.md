...
| `AI_MEMORY_LLM_PROVIDER=opencode` | `claude-sonnet-4-6` | [OpenCode Go](https://opencode.ai/docs/go#endpoints) cloud API at `opencode.ai/zen/go/v1`. GPT-5.6 Luna uses the Responses endpoint; other models use Chat Completions. Requests identify ai-memory by version and reuse one session header across related attempts. Set `OPENCODE_API_KEY` (key from `opencode.ai/auth`). Alias: `opencode-zen`. |
