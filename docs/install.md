...
runs longer than ten minutes without a captured event looks quiet. Reloading
the plugin or restarting the service does not end sessions; deleting one does.
Startup
context is claimed once per root session and retained on every later model
request; child sessions never claim it or publish a baton.
